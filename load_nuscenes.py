...
def get_ground_mesh(ground_points, grid_size=5):
    ground_points = ground_points[
        np.logical_and(ground_points[:, 2] > -1.5, ground_points[:, 2] < 1.5)
    ]
    ...
    def thread_worker(i, region_grid=None, ground_points2grid_map=None, points=None):
        ...
        ground_pcd.points = o3d.utility.Vector3dVector(
            points_[:, :3]
        )  # show_result(points=all_points, gt_bboxes=None, pred_bboxes=None, out_dir="./viz", filename=example['sample_idx'], show=True, snapshot=True)
