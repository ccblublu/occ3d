...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def draw_seg_mask(self, seg_mask_colors: np.ndarray) -> None:
        ...
        seg_points[:, 0] += offset
        self.set_points(seg_points, pcd_mode=2, vis_mode='add', mode='xyzrgb', points_size=30)
        ...
        if vis_task in ['mono_det', 'multi-modality_det']:
            ...
            if isinstance(img, list) or (isinstance(img, (np.ndarray, Tensor))
                ...
                for i, single_img in enumerate(img):
                    ...
                    if raw_image.shape[:2] != img_size:
                        if raw_image.shape[:2] != img_size[::-1]:
                            # resize the image to the original size
                            raw_image = cv2.resize(
                                raw_image, img_size[::-1], interpolation=cv2.INTER_LINEAR)
                        else:
                            raw_image = cv2.resize(
                                raw_image, img_size, interpolation=cv2.INTER_LINEAR)
                    composed_img[(i // img_col) *
                                 img_size[0]:(i // img_col + 1) * img_size[0],
                                 (i % img_col) *
                                 img_size[1]:(i % img_col + 1) *
                                 img_size[1]] = raw_image
    ...
    def _draw_pts_sem_seg(self,
        ...
        if pts_sem_seg.shape[1] == 1:
            pts_color = palette[pts_sem_seg]
        else:
            pts_color = pts_sem_seg
...
if __name__ == '__main__':
    ...
    import open3d as o3d
    import torch
    from mmdet3d.structures import PointData, DepthInstance3DBoxes, Det3DDataSample
    det3d_local_visualizer = Det3DLocalVisualizer()
    # points = np.random.rand(1000, 3)
    # root = "/home/chen/workspace/points_segmentation/PointTransformerV3/Pointcept/runs/vis_pca/point_viz_out"
    root = "/home/chen/workspace/occ/Occ3D/runs/debug"
    # label_list = {}
    ply_list = sorted([i for i in os.listdir(root) if i.endswith(".ply")])
    for file in ply_list:
        pcd = o3d.io.read_point_cloud(os.path.join(root, file))
        points = np.asarray(pcd.points)
        gt_seg = np.asarray(pcd.colors)

        gt_pts_seg = PointData()
        gt_pts_seg.pts_semantic_mask = gt_seg
        gt_det3d_data_sample = Det3DDataSample()
        gt_det3d_data_sample.gt_pts_seg = gt_pts_seg
        data_input = dict(points=points)
        det3d_local_visualizer.add_datasample('3D Scene',
                                              data_input,
                                              gt_det3d_data_sample,
                                              draw_pred=False,
                                              show=True,
